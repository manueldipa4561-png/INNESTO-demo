  ...
  if(!reduceMotion){
    const startScene=async()=>{
      try{
        ...
        groups.forEach((g,i)=>g.userData.base=[...targets[0][i]]);
        ...
        const resize=()=>{const r=stage.getBoundingClientRect();renderer.setSize(Math.max(1,r.width),Math.max(1,r.height),false);camera.aspect=r.width/Math.max(1,r.height);camera.updateProjectionMatrix()};resize();new ResizeObserver(resize).observe(stage);
        ...
        stage.addEventListener('pointermove',e=>{const r=stage.getBoundingClientRect();pointer.x=((e.clientX-r.left)/r.width-.5)*2;pointer.y=((e.clientY-r.top)/r.height-.5)*2;if(drag){targetRotY+=(e.clientX-lastX)*.006;targetRotX=Math.max(-.15,Math.min(.95,targetRotX+(e.clientY-lastY)*.004));lastX=e.clientX;lastY=e.clientY}});
        ...
        const animate=()=>{
          ...
          groups.forEach((g,i)=>{const t=targets[phase][i];g.position.x+=(t[0]-g.position.x)*ease;g.position.y+=(t[1]-g.position.y)*ease;g.position.z+=(t[2]-g.position.z)*ease;const current=(g.userData.opacity??0);const next=current+(opacities[phase][i]-current)*ease;g.userData.opacity=next;setMatOpacity(g,next)});
